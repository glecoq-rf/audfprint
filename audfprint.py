    ...
    if skip_existing and os.path.isfile(opfname):
        ...
        if len(output) == 0:
            ...
            message = (
                "wrote "
                + opfname
                + " ( %d %s, %.3f sec)" % (len(output), type, analyzer.soundfiledur)
            )
        return [message]


def file_precompute(
    analyzer, filename, precompdir, type="peaks", skip_existing=False, strip_prefix=None
):
    """Perform precompute action for one file, return list
    of message strings"""
    print(time.ctime(), "precomputing", type, "for", filename, "...")
    hashes_not_peaks = type == "hashes"
    return file_precompute_peaks_or_hashes(
        analyzer,
        filename,
        precompdir,
...
def do_cmd(
    ...
    if cmd == "merge" or cmd == "newmerge":
        ...
        ix = 0
        for filename in filename_iter:
            report([time.ctime() + " ingesting #" + str(ix) + ": " + filename + " ..."])
...
def multiproc_add(analyzer, hash_tab, filename_iter, report, ncores):
    ...
    for ix in range(ncores):
        ...
        pr[ix] = multiprocessing.Process(
            target=make_ht_from_list,
            args=(
                analyzer,
                filelists[ix],
                hash_tab.hashbits,
    ...
    if cmd == "precompute":
        ...
        msgslist = joblib.Parallel(n_jobs=ncores)(
            joblib.delayed(file_precompute)(
                analyzer, file, outdir, type, skip_existing, strip_prefix=strip_prefix
            )
        ...
        msgslist = joblib.Parallel(n_jobs=ncores)(
            ...
            joblib.delayed(matcher_file_match_to_msgs)(
                matcher, analyzer, hash_tab, filename
            )
            for filename in filename_iter
        )
        for msgs in msgslist:
...
def setup_matcher(args):
    ...
    matcher.exact_count = (
        args["--exact-count"] | args["--illustrate"] | args["--illustrate-hpf"]
    )
    ...
    matcher.report_folder = args["--export"]
    return matcher
...
Options:
  -e <name>, --export <name>      Export results to csv file
...
def main(argv):
    ...
    analyzer = (
        setup_analyzer(args)
        if not (cmd == "merge" or cmd == "newmerge" or cmd == "list" or cmd == "remove")
        else None
    )
    ...
    if cmd != "precompute":
        ...
        if cmd == "new" or cmd == "newmerge":
            ...
            if (
                analyzer
                and "samplerate" in hash_tab.params
    ...
    if ncores > 1 and not (
        cmd == "merge" or cmd == "newmerge" or cmd == "list" or cmd == "remove"
    ):
    ...
    if analyzer and analyzer.soundfiletotaldur > 0.0:
        print(
            "Processed "
            + "%d files (%.1f s total dur) in %.1f s sec = %.3f x RT"
            % (
                analyzer.soundfilecount,
                analyzer.soundfiletotaldur,
                elapsedtime,
